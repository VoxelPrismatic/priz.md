...
return st;}
function sleep(ms) {var endTime = Date.now() + ms;while(endTime > Date.now()) {}}
function load_script(src) {globalThis.src__ = src;console.groupCollapsed("Dynamic load");console.info(`Dynamically loading '${src__}'`);fetch(src).then(resp => resp.text().then(code => {globalThis.eval(code);console.info(`Finished loading '${src__}'`);}))
;sleep(25);console.groupEnd("Dynamic load");}
