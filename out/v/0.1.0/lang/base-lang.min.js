...
;var std_err__ = [[/([\w\d_:]*)([Ee]rror|[Ee]xception|[Ff]ailure|[Ee]xit|[Ww]arning|[Aa]bort)/gm,
...
kw.sort().reverse();cls.sort().reverse();var sym = "[\\"+"\\.,:;()[]{}~|/-+=*^%&@ ".split('').join("\\")+"]";var gsym = "(" + sym + ")";if(edit) {for(var r of kw) {if(r == "class")
r += "[^=]";var r2 = r.replace(/\\/gm, "");st = st.replace(RegExp("^" + r + gsym, "gm"),
...
for(var r of cls) {var r2 = r.replace(/\\/gm, "");st = st.replace(RegExp("^" + r + gsym, "gm"),
`<span class="cls">${r2.split('').join('\u200b')}</span>$1`)
;st = st.replace(RegExp("(" + sym + "|\n|[\u200b ]+)" + r + gsym, "gm"),
`$1<span class="cls">${r2.split('').join('\u200b')}</span>$2`)
;st = st.replace(RegExp("^" + r + "$"),
`<span class="cls">${r2.split('').join('\u200b')}</span>`);}
if(aio) {for(var r of aio_text) {var r2 = r.replace(/\\/gm, "");st = st.replace(RegExp("^" + r + gsym, "gm"),
