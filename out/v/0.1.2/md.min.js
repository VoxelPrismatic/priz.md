...
f.open("GET", src, {async: false});f.send();content = globalThis.resp__
if(strip || list || json || script) {console.log("Trimming content");content = content.trim();}
if(json) {console.log("Parsing JSON");content = JSON.parse(content.replace(/\\\n/gm, "\\n"));}
else if(list) {console.log("Splitting list");content = content.split("\n");}
else if(script) {console.info(`Evaluating '${src__}'`);globalThis.eval(content);}
console.info(`Finished loading '${src__}'`);console.groupEnd("Dynamic load");return content;}
...
function(m, p1, p2) {return `<def title="${esc(p2)}">${esc(mark(p1))}</def>`;}], [/\@\[(.+?)\]<(.+?)>\((.*)\)/gm,
function(m, p1, p2, p3) {var alt = esc(mark(p1).replace(/"/gm, "'"));return `<img alt="${alt}" title="${alt}" width="${p3}" src="${esc(p2)}">`;}], [/\+\[\[(.+?)\]\]\<(.+?)\>\((.*?)\)/gm,
function(m, p1, p2, p3) {return `<a href="${esc(p2)}" title="${esc(p3)}" target='\\x5fblank'><span class='btn'>${esc(mark(p1))}</span></a>`;}], [/\+\[(.+?)\]\<(.+?)\>\((.*)\)/gm,
function(m, p1, p2, p3) {return `<a href="${esc(p2)}" title="${esc(p3)}" target='\\x5fblank'>${esc(mark(p1))}</a>`;}], [/\[\[(.+?)\]\]\<(.+?)\>\((.*)\)/gm,
function(m, p1, p2, p3) {return `<a href="${esc(p2)}" title="${esc(p3)}"><span class='btn'>${esc(mark(p1))}</span></a>`;}], [/\[(.+?)\]\<(.+?)\>\((.*)\)/gm,
function(m, p1, p2, p3) {return `<a href="${esc(p2)}" title="${esc(p3)}">${esc(mark(p1))}</a>`;}], [/\@\[(.+?)\]<(.+?)>/gm,
function(m, p1, p2) {var alt = esc(mark(p1).replace(/"/gm, "'"));return `<img alt="${alt}" title="${alt}" src="${esc(p2)}">`;}], [/\+\[\[(.+?)\]\]\<(.+?)\>/gm,
...
[/\>\:(.+?)\:\</gm,
function(m, p) {load_unicode_index__();return "\\u{" + unimap(p.replace(/_/gm, " ")) + "}";}],
[/(.*)\:\^\:(.*)/gm,
