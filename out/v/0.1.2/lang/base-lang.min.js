...
function(m, p1, p2) {return `<span class="${css}">${p1.split('').join('\u200b')}</span>${p2}`;})
;if(!r_og.includes("class"))
st = st.replace(RegExp("(" + sym + "|\n|[\u200b ]+)" + r + gsym, "gm"),
function(m, p1, p2, p3) {return `${p1}<span class="${css}">${p2.split('').join('\u200b')}</span>${p3}`;})
