...
return st;}
function load_script(src) {globalThis.src__ = src;console.groupCollapsed("Dynamic load");console.info(`Reading '${src__}'`);var f = new XMLHttpRequest()
f.onreadystatechange = function() {if (this.readyState == 4 && this.status == 200) {globalThis.resp__ = f.responseText;}}
f.open("GET", src, false);f.send();console.info(`Evaluating '${src__}'`);globalThis.eval(globalThis.resp__);console.info(`Finished loading '${src__}'`);console.groupEnd("Dynamic load");}
function mark_page(st) {if(!line_regex__[0])
set_regex__();if(!(st.endsWith("\n")))
st += "\n";st = st.replace(/\\ *\n/gm, "");var str = "";var py = "";var table = "";var ol = "";var ul = "";var al = "";var quoted = "";var dropper = "";var syntax = "";var code = "";var incode = false;var indropper = false;var intable = false;var moresyntax = false;for(var line of st.split("\n")) {
...
catch(err) {load_script(`${base__}lang/base-lang.min.js`);load_script(`${base__}lang/index-lang.min.js`);stylesheet__ = document.createElement("link");stylesheet__.rel = "stylesheet";stylesheet__.type = "text/css";stylesheet__.href = base__ + "syntax.min.css";document.head.append(stylesheet__);}
if(typeof fn == "string")
syntax = syntax_alias__[syntax];fn = syntax_alias__[syntax];if(fn == undefined) {load_script(`${base__}lang/${syntax}-lang.min.js`);if(syntax == "html") {load_script(`${base__}lang/js-lang.min.js`);load_script(`${base__}lang/css-lang.min.js`);}
