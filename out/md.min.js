...
function(m, p1, p2) {return `<img alt="${esc(mark(p1).replace(/"/gm, "'"))}" width="${p3}" src="${esc(p2)}">`;}], [/\@\[(.+?)\]<(.+?)>\((.*)\)/gm,
function(m, p1, p2, p3) {return `<img alt="${esc(mark(p1).replace(/"/gm, "'"))}" src="${esc(p2)}">`;}], [/\+\[\[(.+?)\]\]\<(.+?)\>\((.*?)\)/gm,
function(m, p1, p2, p3, p3) {return `<a href="${esc(p2)}" title="${esc(p3)}" target='\\x5fblank'><span class='btn'>${esc(mark(p1))}</span></a>`;}], [/\+\[(.+?)\]\<(.+?)\>\((.*)\)/gm,
