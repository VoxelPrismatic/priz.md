...
"\u0321",
"\u0327",
"\u0309"];if(!ok_accents.includes(acc.innerHTML)) {acc.style.left = (-char.clientWidth / 2) + "px";}
else {acc.style.left = "";}
...
st += "\n";st = st.replace(/\\ *\n/gm, "");recur__.push(".");var str = "";var py = "";var table = "";var ol = "";var ul = "";var al = "";var quoted = "";var dropper = "";var syntax = "";var code = "";var incode = false;var indropper = false;var intable = false;var moresyntax = false;for(var line of st.split("\n")) {
...
if(indropper) {if(line.match(/^\`\`\`(\w+)?/gm)) {incode = !incode;}
dropper += line + "\n";continue;}
if(line.match(/^\`\`\`(\w+)?/gm)) {incode = !incode;if(incode) {syntax = line.slice(3).trim();}
else {str += `<codeblock>`;if(syntax) {try {try {var fn = syntax_alias__[syntax];}
...
catch(err) {char = uni_index__[st.toUpperCase()];if(esc) {return "\\u{" + char + "}";}
try {return eval(`"\\u{${char}}"`);}
catch(err) {return st;}}}
...
p2 = "ı";if(p2 == "j")
p2 = "ȷ";return `<span>${p2}</span><accent>${accent}</accent>`;}],
[/\\u\{([a-fA-F0-9]+)\}/gm,
