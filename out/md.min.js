...
return st;}
function mark_page(st) {if(!line_regex__[0]) {set_regex__();}
if(!(st.endsWith("\n")))
...
if(!incode && line && line.replace(/^\>\>\[.*\]\<\<$/gm, "") == "") {indropper = true;}
if(!incode && line == "---" && indropper) {indropper = false;str += mk_dropper__(dropper.slice(0, -1));dropper = "";continue;}
if(indropper) {dropper += line + "\n";continue;}
if(line.match(/^\`\`\`(\w+)?/gm)) {incode = !incode;if(incode) {syntax = line.slice(3);}
else {str += `<div class="code">`;try {var fn = syntax_alias__[syntax];if(typeof fn == "string") {fn = syntax_alias__[fn];}
str += fn(code);}
...
if(line && line.replace(/^\w+[\]\)\.\-] .*$/gm, "") == "") {al += line.replace(/^\d+[\]\)\.\-] /gm, "") + "\n";continue;}
...
str += line+"\n";}
str = str.replace(/\n/gm, "<br>");str = str.replace(/\<((\/)?(h\d|div|ol|ul))\>([ \n]*\<br\>[ \n]*)+/gm, "<$1$2><br>");str = str.replace(/([ \n]*\<br\>[ \n]*)+\<((\/)?(h\d|div|ol|ul))\>/gm, "<br><$2$3>");str = str.replace(/(<br>)*?<(\/)blockquote>(<br>)*?/gm, "<$2blockquote>");return str;}
...
else {table_str[0].push(trim(header));table_aligns.push("justified");}}}
